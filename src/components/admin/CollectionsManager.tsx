  ...
  const handleImageUpload = async (files: FileList | null) => {
    ...
    try {
      const uploadPromises = Array.from(files).map(async (file) => {
        ...
        const result = await uploadFile(file, 'website-images', 'collections');
        if (result.error) {
        ...
        {showAddForm && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
            <h2 className="text-lg font-medium text-gray-900 mb-4">
              {editingId ? 'Редактировать коллекцию' : 'Добавить коллекцию'}
            </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none 
                             focus:ring-1 focus:ring-gray-900 focus:border-gray-900"
                    placeholder="Например: Весна-Лето 2024"
                  />
                </div>
              </div>
          ...
          <div className="divide-y divide-gray-200">
                  ...
                  <div className="flex space-x-4">
                    ...
                    <div>
                      <h3 className="text-lg font-medium text-gray-900 mb-1">
                        {collection.title}
                        {collection.featured && (
                          <span className="ml-2 inline-block bg-yellow-100 text-yellow-800 
                                         text-xs px-2 py-1 rounded-full">
                            Рекомендуемое
                          </span>
                        )}
                      </h3>
                      <p className="text-sm text-gray-600 mb-2">{collection.season}</p>
                      <p className="text-sm text-gray-700">{collection.description}</p>
                      <p className="text-xs text-gray-500 mt-2">
                        {collection.images.length} изображений
                      </p>
                    </div>
                  </div>
            ...
            {collections.length === 0 && (
              <div className="p-12 text-center">
                <p className="text-gray-500">Коллекции пока не добавлены</p>
              </div>
            )}
          </div>
        </div>
